document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentSelection = {
        duration: 2,
        label: 'Sommet Classique 2 Jours',
        groupSize: '2-3',
        season: 'summer'
    };
    ...
    function updateBreakdown() {
        ...
        if (groupSizeElement) {
            groupSizeElement.textContent = `${currentSelection.groupSize} personnes`;
        }
        
        if (seasonElement) {
            const seasonText = currentSelection.season === 'summer' ? 'Été' : 'Hiver';
            seasonElement.textContent = seasonText;
        }
    }
    ...
    function updateStickyBooking() {
        ...
        if (stickyItinerary) {
            stickyItinerary.textContent = `${currentSelection.duration} Jours: ${currentSelection.label}`;
        }
        
        if (stickyMeta) {
            const seasonText = currentSelection.season === 'summer' ? 'Été' : 'Hiver';
            stickyMeta.textContent = `${currentSelection.groupSize} personnes · ${seasonText}`;
        }
    ...
    function handleFormSubmission(e) {
        ...
        // Create WhatsApp message in French
        const message = encodeURIComponent(
            `🏔️ Demande de Réservation Trek Mount Toubkal\n\n` +
            `📅 Date Préférée: ${data.travelDate}\n` +
            `⏱️ Itinéraire: ${currentSelection.duration} Jours - ${currentSelection.label}\n` +
            `👥 Taille du Groupe: ${currentSelection.groupSize} personnes (${data.travelers} voyageurs)\n` +
            `🌤️ Saison: ${seasonText}\n` +
            `💰 Prix: €${price} par personne\n` +
            `👤 Nom: ${data.name}\n` +
            `📧 Email: ${data.email}\n` +
            `📞 Téléphone: ${data.phone}\n` +
            `📝 Demandes Spéciales: ${data.message || 'Aucune'}\n\n` +
            `Veuillez confirmer la disponibilité et procéder à la réservation.`
        );
        ...
        submitBtn.innerHTML = '<span class="cta-text">✓ Demande Envoyée</span>';
